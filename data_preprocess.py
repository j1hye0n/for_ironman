...
for k in range(len(case_index)):
    ...
    case=pd.read_json(r'CASE/case_'+str(case_index[k])+'/case_'+str(case_index[k])+'_all_data.json')
    ...
    for i in range(len(LUT_dir)):
        #print(f"LUT_dir={LUT_dir[i]}")
        solution=LUT_dir[i]
        f10=[0 for i in range(len(node_features))]
        #print(f"solution type = {type(solution)}, solution = {solution}")
